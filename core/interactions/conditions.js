...
exports.untilIsEnabled = async (locator) => {
  const element = await this.myFindElement(locator);
  myLogger.info(
    `Checking if element is enabled with locator: ${locator.value}`
  );
...
exports.findUntilIsVisible = async (locator, timeout = 1000) => {
  try {
    ...
    myLogger.error(
      `Error: No se pudo encontrar o hacer visible el elemento con el locator ${locator.value} en el tiempo de espera.`
    );
    throw new Error(
      `El elemento con el locator ${locator.value} no es visible dentro del tiempo permitido.`
    );
  }
};
...
exports.untilElementDoesNotExist = async (locator) => {
  myLogger.info(
    `Checking if element does NOT exist with locator: ${locator.value}`
  );
  ...
  try {
    ...
    if (elements.length === 0) {
      myLogger.info(
        `Element with locator: ${locator.value} does not exist in the DOM.`
      );
      return true; // El elemento no existe
    } else {
      myLogger.info(
        `Element with locator: ${locator.value} is still present in the DOM.`
      );
